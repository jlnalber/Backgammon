...
export function downloadJSONAsFile(object: any, fileName: string) {
  const content = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(object));
...
export function roundToDigitsWithUndefined(num?: number, digits?: number): number | undefined {
  if (num === undefined) {
    return undefined;
  }
...
export function roundToDigits(num: number, digits?: number): number {
  if (digits === undefined) {
    return num;
  }
...
export function roundPointToDigits(point: Point, digits?: number): Point {
  return {
    x: roundToDigits(point.x, digits),
    y: roundToDigits(point.y, digits)
  }
}
